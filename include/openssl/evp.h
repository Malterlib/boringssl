...
// - `N` > 2^32
// - `N` > 2^(128 * `r` / 8)
OPENSSL_EXPORT int EVP_PBE_scrypt_with_digest(
    const char *password, size_t password_len, const uint8_t *salt,
    size_t salt_len, uint64_t N, uint64_t r, uint64_t p, size_t max_mem,
    const EVP_MD *digest, uint8_t *out_key, size_t key_len);
