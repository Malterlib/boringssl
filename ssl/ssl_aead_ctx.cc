...
bool SSLAEADContext::OpenScatterV(size_t *out_len, uint8_t type,
  ...
  if (is_null_cipher() || CRYPTO_fuzzer_mode_enabled()) {
    // Pairing both lists may require `2 * CRYPTO_IOVEC_MAX - 1` entries.
    Span<const CRYPTO_IOVEC> pairs;
    CRYPTO_IOVEC pair_storage[2 * CRYPTO_IOVEC_MAX];
  ...
  // Detach fixed-size tags so output capacity need only cover plaintext.
  size_t tag_len;
  ...
  // Variable overhead requires output space for the whole encrypted body.
  Span<const CRYPTO_IOVEC> pairs;
  CRYPTO_IOVEC pair_storage[2 * CRYPTO_IOVEC_MAX];
...
bool SSLAEADContext::FixedTagLen(size_t *out_tag_len) const {
  if (is_null_cipher() || CRYPTO_fuzzer_mode_enabled() ||
    ...
    // Null ciphers have no tag; CBC padding is known only after decryption.
    return false;
  }
