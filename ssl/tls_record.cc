...
// Keep record validation and its ordering in sync with `tls_open_record`.
enum ssl_open_record_t tls_open_record_v(SSLImpl *ssl, uint8_t *out_type,
  ...
  *out_consumed = record_len;
  ...
  // Allow alerts through partial handshake state, matching `tls_open_record`.
  // The caller processes them after this function returns.
  ...
  // Do not reset the warning count for alerts: the caller processes them
  // after this returns, and resetting here would bypass the limit.
...
bool tls_seal_record_v(SSLImpl *ssl, uint8_t *out, size_t *out_len,
  ...
  // TLS 1.0 CBC splitting requires the contiguous record path.
  if (type == SSL3_RT_APPLICATION_DATA && in_len > 1 &&
      ssl_needs_record_splitting(ssl)) {
